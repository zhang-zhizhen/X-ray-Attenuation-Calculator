...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    if (nrhs == 2 && nlhs != 1)
    {
        mexErrMsgTxt("Output Error : loss an output\n");
    }
    ...
    else
    {
        ...
        {
            if (mxGetM(prhs[1]) != 1u && mxGetN(prhs[1]) != 1u)
            {
                mexErrMsgTxt("Input Error : second input dimensions error and is not a 1 by N (N by 1) double matrix\n");
            }
            else
            {
                size_t NumOfPoints = mxGetM(prhs[1]) * mxGetN(prhs[1]);
                //float SCTCO[ME], SCTIN[ME], PHT[ME], PRAT[ME], PREL[ME], PHDIF[ME];
